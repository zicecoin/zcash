...
#
# In this test we connect Sprout, Overwinter, and Sapling mininodes to a ZiCEd
# node which will activate Overwinter at block 10 and Sapling at block 15.
#
# We test:
# 1. the mininodes stay connected to ZiCE with Sprout consensus rules
# 2. when Overwinter activates, the Sprout mininodes are dropped
# 3. new Overwinter and Sapling nodes can connect to ZiCE
# 4. new Sprout nodes cannot connect to ZiCE
# 5. when Sapling activates, the Overwinter mininodes are dropped
# 6. new Sapling nodes can connect to ZiCE
# 7. new Sprout and Overwinter nodes cannot connect to ZiCE
#
# This test *does not* verify that prior to each activation, the ZiCEd
...
class NUPeerManagementTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify mininodes are still connected to ziced node
        ...
        # Mininodes send ping message to ziced node.
        pingCounter = 1
        for node in nodes:
        ...
        # Connect a new Overwinter mininode to the ziced node, which is accepted.
        ...
        # Connect a new Sapling mininode to the ziced node, which is accepted.
        ...
        # Try to connect a new Sprout mininode to the ziced node, which is rejected.
        ...
        # Try to connect a new Overwinter mininode to the ziced node, which is rejected.
