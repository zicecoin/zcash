#!/usr/bin/env python3
#
# Execute all of the automated tests related to ZiCE.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/zice/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/zice/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if ziced is an ELF binary, they all are
    with open(repofile('src/ziced'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/ziced')
    ret &= test_rpath_runpath('src/zice-cli')
    ret &= test_rpath_runpath('src/zice-gtest')
    ret &= test_rpath_runpath('src/zice-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/ziced')
    ret &= test_fortify_source('src/zice-cli')
    ret &= test_fortify_source('src/zice-gtest')
    ret &= test_fortify_source('src/zice-tx')
...
STAGE_COMMANDS = {
    'check-depends': ['qa/zice/test-depends-sources-mirror.py'],
    'rust-test': rust_test,
    'btest': [repofile('src/test/test_bitcoin'), '-p'],
    'gtest': [repofile('src/zice-gtest')],
