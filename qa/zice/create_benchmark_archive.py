...
ZICE_CLI = './src/zice-cli'
USAGE = """
Requirements:
- find
- xz
- %s (edit ZICE_CLI in this script to alter the path)
- A running mainnet ziced using the default datadir with -txindex=1
...
LD_LIBRARY_PATH=src/leveldb python qa/zice/create_benchmark_archive.py
""" % ZICE_CLI

def check_deps():
    if subprocess.call(['which', 'find', 'xz', ZICE_CLI], stdout=subprocess.PIPE):
        print(USAGE)
        sys.exit()
...
def create_benchmark_archive(blk_hash):
    blk = json.loads(subprocess.check_output([ZICE_CLI, 'getblock', blk_hash]))
    print ('Height: %d' % blk['height'])
    print ('Transactions: %d' % len(blk['tx']))

    os.mkdir('benchmark')
    with open('benchmark/block-%d.dat' % blk['height'], 'wb') as f:
        f.write(binascii.unhexlify(subprocess.check_output([ZICE_CLI, 'getblock', blk_hash, 'false']).strip()))

    txs = [json.loads(subprocess.check_output([ZICE_CLI, 'getrawtransaction', tx, '1'])
                     ) for tx in blk['tx']]
    ...
    for tx in bar(unique_inputs.keys()):
        rawtx = json.loads(subprocess.check_output([ZICE_CLI, 'getrawtransaction', tx, '1']))
        ...
        coins.extend(encode_varint(json.loads(
            subprocess.check_output([ZICE_CLI, 'getblockheader', rawtx['blockhash']])
            )['height']))
