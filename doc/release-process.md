...
If this is a hotfix release, please see the [Hotfix Release Process](https://github.com/zice/zice/blob/master/doc/hotfix-process.md) documentation before proceeding.
...
Push the resulting branch to github:
    ...
    $ git push 'git@github.com:$YOUR_GITHUB_NAME/zice' $(git rev-parse --abbrev-ref HEAD)
...
- Run the [Gitian deterministic build environment](https://github.com/zice/zice-gitian)
- Compare the uploaded [build manifests on gitian.sigs](https://github.com/zice/gitian.sigs)
- If all is well, the DevOps engineer will build the Debian packages and update the
  [apt.z.cash package repository](https://apt.z.cash).

## Add release notes to GitHub

- Go to the [GitHub tags page](https://github.com/zice/zice/tags).
...
Notify the ZiCE DevOps engineer/sysadmin that the release has been tagged. They update some variables in the company's automation code and then run an Ansible playbook, which:

* builds ZiCE based on the specified branch
* deploys it as a public service (e.g. betatestnet.z.cash, mainnet.z.cash)
* often the same server can be re-used, and the role idempotently handles upgrades, but if not then they also need to update DNS records
* possible manual steps: blowing away the `testnet3` dir, deleting old parameters, restarting DNS seeder

Then, verify that nodes can connect to the testnet server, and update the guide on the wiki to ensure the correct hostname is listed in the recommended zice.conf.

### Update the 1.0 User Guide

This also means updating [the translations](https://github.com/zice/zice-docs).
Coordinate with the translation team for now. Suggestions for improving this
part of the process should be added to #2596.

### Publish the release announcement (blog, github, zice-dev, slack)
