...
Shorter Block Times
-------------------
Shorter block times are coming to ZiCE! In the v2.0.7 release we have implemented [ZIP208](https://github.com/zice/zips/blob/master/zip-0208.rst) which will take effect when Blossom activates. Upon activation, the block times for ZiCE will decrease from 150 seconds to 75 seconds, and the block reward will decrease accordingly. This affects at what block height halving events will occur, but should not affect the overall rate at which ZiCE is mined. The total founders' reward stays the same, and the total supply of ZiCE is decreased only microscopically due to rounding.
...
Insight Explorer
----------------
Changes needed for the Insight explorer have been incorporated into ZiCE. *This is an experimental feature* and therefore is subject to change. To enable, add the `experimentalfeatures=1`, `txindex=1`, and `insightexplorer=1` flags to `zice.conf`. This feature adds several RPCs to `ziced` which allow the user to run an Insight explorer.
