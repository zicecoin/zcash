...
[ZIP208](https://github.com/zice/zips/blob/master/zip-0208.rst) which shortens
...
One of the mechanisms that `ziced` uses to detect whether it is in "initial
...
Starting from this release, `ziced` additionally hard-codes the block hashes
...
surface of `ziced`, we are removing `libsnark` from the codebase, and dropping
...
- `ziced` no longer verifies old Sprout proofs, and will instead assume they
...
- Shielded transactions can no longer be created before Sapling has activated.
  This does not affect ZiCE itself, but will affect downstream codebases that
...
- Starting from this release, the circuit parameters from the original Sprout
  MPC are no longer required to start `ziced`, and will not be downloaded by
...
It was vital to the success of ZiCE, and the development of zero-knowledge
