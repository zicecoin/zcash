...
Nathan Wilcox (4):
      ...
      Add a zice-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the zice-gtest run; invoke with make zice-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to zice-gtest.
      ...
      Small nit fixes
      Run `zice-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libzcash` ZiCE protocol API and crypto constructions surrounding the zkSNARK circuit.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to zice.conf and ziced.pid in qa/ and src/
