...
Jay Graber (17):
      ...
      Refer to ZiCE wiki in INSTALL
      Rm bitcoin logo
      Rm build-unix.md, to keep single copy of build instructions for ZiCE on github wiki
      Rm Bitcoin-specific documentation
      Add note that document is not updated for ZiCE to translation policy
      Rm doc for disabled REST interface
      Change alpha to beta testnet, add zice hidden service
      Improve documentation on connecting to zice hidden server
      Improve documentation on connecting to zice hidden server
      Update tor.md
...
Kevin Gallagher (24):
      ...
      Adds zice/ZiCE.h to LIBZICE sources
      Adds zice/Proof.hpp to LIBZICE_H
      Add alertkeys.h to libbitcoin_server_a_SOURCES
      Adds files in src/zice/circuit to libzcash_a_SOURCES
      Adds zcbenchmarks.h to libbitcoin_wallet_a_SOURCES
      Adds json_test_vectors.h to zice_gtest_SOURCES
      Adds additional licenses to Debian copyright file
      Updates ZiCE Core developers -> ZiCE developers
