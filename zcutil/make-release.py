...
def main(args=sys.argv[1:]):
    """
    Perform the final ZiCE release process up to the git tag.
    """
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "zice-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "zice-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    ...
    logging.info('zice make-release.py debug log: %r', logname)
