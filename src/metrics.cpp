...
int printMiningStatus(bool mining)
{
    ...
    if (mining) {
        ...
        std::cout << _("To enable mining, add 'gen=1' to your zice.conf and restart.") << std::endl;
        lines += 2;
    }
    std::cout << std::endl;
...
void ThreadShowMetricsScreen()
{
    ...
    RenameThread("zice-metrics-screen");
    ...
    if (isScreen) {
        ...
        // Thank you text
        std::cout << strprintf(_("Thank you for running a %s ziced v%s node!"), WhichNetwork(), FormatVersion(CLIENT_VERSION)) << std::endl;
    ...
    while (true) {
        ...
        if (isScreen) {
            ...
            std::cout << "[";
#ifdef WIN32
            std::cout << _("'zice-cli.exe stop' to exit");
#else
