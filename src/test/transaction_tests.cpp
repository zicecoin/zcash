...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    {
        JSDescription jsdesc(*pziceParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
...

    {
        ...
        BOOST_CHECK_THROW(JSDescription(*pziceParams, joinSplitPubKey, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pziceParams, joinSplitPubKey, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pziceParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
