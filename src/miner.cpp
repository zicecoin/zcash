...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZiCEMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL))
        return error("ZiCEMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ZiCEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zice-miner");
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in ZiCEMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in ZiCEMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running ZiCEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ZiCEMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ZiCEMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
