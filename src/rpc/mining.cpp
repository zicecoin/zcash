...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "ziced compiled without wallet and -mineraddress not set");
#endif
    }
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ZiCE is not connected!");

    if (IsInitialBlockDownload(Params()))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ZiCE is downloading blocks...");
