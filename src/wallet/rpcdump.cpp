...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"ziceprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"ziceprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZiCE address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid ZiCE spending key
        if (fImportZKeys) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                // Not a valid spending key, so carry on and see if it's a ZiCE style t-address.
            }
        }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by ziced -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the ziced -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by ZiCE %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
