...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("ZiCE zice-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  zice-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded zice transaction") + "\n" +
              "  zice-tx [options] -create [commands]   " + _("Create hex-encoded zice transaction") + "\n" +
              "\n";
